...
export function Button({ children, onPress, loading = false, disabled = false, variant = 'primary', icon, style }: ButtonProps) {
  ...
  return (
    <Pressable
      ...
      {loading ? (
        ...
        <>
          {icon ? <Ionicons name={icon} size={19} color={variant === 'primary' ? colors.white : colors.primary} /> : null}
          <Text numberOfLines={2} ellipsizeMode="tail" style={[styles.buttonText, styles[`buttonText_${variant}`]]}>{children}</Text>
        </>
      )}
    </Pressable>
  );
...
type FieldProps = TextInputProps & { label: string; error?: string; keyboardType?: KeyboardTypeOptions };

export const Field = forwardRef<TextInput, FieldProps>(function Field({ label, error, multiline, style, ...props }, ref) {
  return (
...
export function StatusBadge({ status }: { status: OrderStatus }) {
  ...
  return <View style={[styles.statusBadge, { backgroundColor: statusStyle.backgroundColor }]}><Text style={[styles.statusText, { color: statusStyle.color }]}>{orderStatusLabel[status]}</Text></View>;
}

export function EmptyState({ icon, title, message, actionLabel, onAction }: { icon: keyof typeof Ionicons.glyphMap; title: string; message: string; actionLabel?: string; onAction?: () => void }) {
  return (
    <View style={styles.empty}>
      <View style={styles.emptyIcon}><Ionicons name={icon} size={34} color={colors.primary} /></View>
      <Text style={styles.emptyTitle}>{title}</Text>
      <Text style={styles.emptyMessage}>{message}</Text>
      {actionLabel && onAction ? <Button onPress={onAction} style={styles.emptyButton}>{actionLabel}</Button> : null}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  button_secondary: { borderWidth: 1, borderColor: colors.primary, backgroundColor: colors.surface },
  button_danger: { borderWidth: 1, borderColor: colors.danger, backgroundColor: colors.surface },
